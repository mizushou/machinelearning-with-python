...
# Problem 9. Remove Stop Words
with open('stopwords.txt') as f:
    stopwords_data = list(f)
    
## stoprwords_dataの各要素に\nがついてしまうので、\nを削除。mapだと戻り値がlistでなくmap型になってしまうので、下記のlist内記法に変更
# stopwords_data = map(lambda s: s.strip(), stopwords_data)
stopwords_data = [s.strip() for s in stopwords_data]
...
dictionary = p1.bag_of_words_remove_stop_words(train_texts, stopwords_data)
...
best_theta, theta_0 = \
   p1.classifier_parameter(p1.pegasos, train_bow_features,test_bow_features,train_labels,test_labels,T=T,L=L)
